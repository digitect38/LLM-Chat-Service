...
8. [로그 분석기 (Log Analyzer)](#8-로그-분석기)
9. [오류 및 문제 해결](#9-오류-및-문제-해결)
10. [관리자 설정 가이드](#10-관리자-설정-가이드)
11. [변경 이력](#11-변경-이력)
...
- RAG 기반 지식 검색 (매뉴얼, SOP, 트러블슈팅 문서 자동 참조 + **출처 인용**)
...
화면은 **4개 영역**으로 구성되며, 오른쪽 패널 2개(Log Analyzer, Citation)는 독립적으로 토글할 수 있습니다.

```
+----------+------------------+------------+------------+
| Sidebar  |   Chat Area      | Log Pane   | Citation   |
| (260px)  |   (flex:1)       | (380px)    | (350px)    |
|          |                  |            |            |
| History  |   Top Bar        | Search Bar | 참조 문서  |
| 장비선택 |   [JIT][Fade]    | Stats      |  목록     |
|          |   [Log][Cite]    | Sub-tabs   |  PDF뷰어  |
|          |                  | Results    |            |
|          |   Messages       |            |            |
|          |                  |            |            |
|          |   Input Box      |            |            |
...
> Log Analyzer 패널과 Citation 패널은 **동시에 열 수 있으며**, 각각 독립적으로 토글됩니다.

### 2.1 사이드바 (Sidebar)

| 요소 | 설명 |
|------|------|
| **대화 기록** | 이전 대화 세션 목록 (클릭하여 전환) |
| **장비 선택** | 드롭다운으로 장비 ID 선택 (예: `CMP01`, `ETCH01`) |
| **연결 상태** | 초록색 점 = 연결됨, 회색 점 = 미연결 |
| **설정 / 서버 설정** | 설정 페이지 이동 버튼 |

### 2.2 상단 바 (Top Bar)

| 요소 | 설명 |
|------|------|
| **렌더 모드** | JIT / Fade / Batch 스트리밍 렌더링 방식 선택 |
| **Log Analyzer 아이콘** | 로그 분석 패널 토글 (차트 아이콘) |
| **Citation 아이콘** | 참조 문서 패널 토글 (문서 아이콘) |

### 2.3 채팅 영역

- **사용자 메시지** (오른쪽): 아바타 + 메시지 텍스트
- **Copilot 응답** (왼쪽): 아바타 + Markdown 렌더링 응답
- 응답 생성 중에는 **깜빡이는 커서** 가 표시됩니다
- 응답 하단 **액션 버튼**: 복사, 좋아요/싫어요, TTS 재생, 참조 문서, 재생성
...
- **파일 업로드**: 클립 아이콘 (PDF, MD, TXT, 이미지 지원)
- **음성 입력**: 마이크 아이콘 (STT 음성→텍스트 변환)
- **Voice Mode**: 파형 아이콘 (연속 음성 대화 모드)

### 2.5 오른쪽 패널

채팅 영역 우측에 두 개의 슬라이딩 패널이 있습니다.

| 패널 | 기본 너비 | 토글 방법 | 설명 |
|------|-----------|-----------|------|
| **Log Analyzer** | 380px | Top Bar 차트 아이콘 | 서비스 로그 검색/분석 |
| **Citation** | 350px | Top Bar 문서 아이콘 또는 응답 참조 버튼 | RAG 참조 문서 상세 보기 |

- 두 패널 모두 **리사이즈 핸들**로 너비 조절 가능 (최소 300px ~ 최대 60vw)
- **모바일 (<768px)**: 전체 화면 오버레이로 표시
...
### 7.5 출처 인용 (Source Citation)

RAG 기반 응답 시, Copilot은 **참고한 문서의 출처를 자동으로 표시**합니다.

#### 인용 형식

```
📄 cmp-troubleshooting.md에 따르면, Head Zone 7의 압력 진동은...

---
📚 **참고 문서:**
- cmp-troubleshooting.md (유사도: 0.892)
```
...
| 상황 | Copilot 동작 |
|------|-------------|
| RAG 문서가 관련 있을 때 | `📄 [파일명]에 따르면...` 형식으로 인용하며 답변 |
| RAG 문서가 관련 없을 때 | "참고 문서에 관련 정보가 없어 일반 지식을 바탕으로 답변합니다." 명시 |
| 여러 문서 참조 시 | 각 문서별 인용 + 답변 하단에 참고 문서 목록 표시 |

#### 메타데이터 표시

출처 인용에는 다음 메타데이터가 사용됩니다 (우선순위 순):

1. `file_name` — 파일명 (예: `cmp-troubleshooting.md`)
2. `file_path` — 상대 경로 (예: `subfolder/guide.md`)
3. `document_id` — 문서 ID
4. 내부 ID (위 항목이 모두 없을 경우)

> **참고:** FileWatcher로 수집된 문서는 `file_name`, `file_path` 메타데이터가 자동으로 저장되므로 항상 정확한 파일명이 표시됩니다.

### 7.6 폴더 감시 자동 수집 (File Watcher)
...
## 8. 로그 분석기 (Log Analyzer)

채팅 화면 오른쪽의 **Log Analyzer 패널**을 통해 서비스 로그를 실시간으로 검색하고 분석할 수 있습니다.
Dashboard(5020)와 독립적으로 동작하며, 채팅 중에도 바로 로그를 확인할 수 있습니다.

### 8.1 패널 열기/닫기

1. 상단 바(Top Bar)의 **차트 아이콘**을 클릭합니다
2. 오른쪽에서 380px 너비의 Log Analyzer 패널이 슬라이드됩니다
3. 다시 클릭하면 패널이 닫힙니다
4. 패널 왼쪽 가장자리를 드래그하여 **너비를 조절**할 수 있습니다 (300px ~ 60vw)

> Citation 패널과 동시에 열 수 있습니다.

### 8.2 검색 기능

패널 상단의 검색 바에서 다양한 조건으로 로그를 검색합니다.

| 필터 | 설명 |
|------|------|
| **키워드** | 로그 메시지 전문 검색 (Enter 키로 즉시 검색) |
| **Service** | 특정 서비스만 필터링 (All Services / 개별 선택) |
| **Level 토글** | ERR(빨강), WRN(노랑), INF(파랑), DBG(회색) 개별 on/off |
| **시간 범위** | 1h / 6h / 24h / 7d 프리셋 버튼 |
| **CorrelationId** | 특정 요청 흐름만 추적 |

### 8.3 탭 기능

검색 바 아래에 **4개 탭**이 있습니다.

#### Results (검색 결과)
- 검색 조건에 맞는 로그를 시간순(최신 우선)으로 표시합니다
- 테이블 열: Time, Service, Level, CorrelationId, Message
- CorrelationId를 클릭하면 자동으로 **Flow 탭**으로 전환됩니다
- 100건 단위 **페이지네이션** 지원

#### Errors (Top 에러)
- 선택한 시간 범위의 에러를 **MessageTemplate 기준으로 그룹화**합니다
- 발생 횟수(Count), 서비스명, 최초/최종 발생 시각 표시
- 반복 발생하는 에러 패턴을 빠르게 파악할 수 있습니다

#### Dashboard (서비스 현황)
- 서비스별 로그 건수를 **수평 막대 그래프**로 시각화합니다
- 색상 구분: 빨강(ERR), 노랑(WRN), 파랑(INF/DBG)
- 어떤 서비스에 에러가 집중되는지 한눈에 파악 가능

#### Flow (요청 흐름)
- **CorrelationId**를 입력하면 해당 요청이 서비스를 거친 순서를 **타임라인**으로 표시합니다
- 각 이벤트: 서비스명, 시각, 이전 이벤트와의 시간 차(gap), 메시지
- 크로스-서비스 요청 흐름을 추적하여 **병목 지점**을 찾을 수 있습니다

### 8.4 통계 요약

검색 실행 후 탭 위에 **4개 통계 카드**가 표시됩니다.

| 카드 | 설명 |
|------|------|
| **Total** | 검색 조건에 맞는 전체 로그 건수 |
| **Errors** | 현재 페이지의 ERR 레벨 건수 |
| **Warnings** | 현재 페이지의 WRN 레벨 건수 |
| **CIDs** | 현재 페이지의 고유 CorrelationId 수 |

### 8.5 활용 예시

| 상황 | 사용 방법 |
|------|-----------|
| 특정 에러 메시지 찾기 | 키워드에 에러 메시지 일부 입력 → ERR만 토글 → 검색 |
| 최근 1시간 에러 현황 | 시간 범위 `1h` → Errors 탭 |
| 서비스별 에러 비중 | 시간 범위 선택 → Dashboard 탭 |
| 느린 요청 추적 | 채팅 로그에서 CorrelationId 복사 → Flow 탭에 붙여넣기 → 추적 |
| 특정 서비스 집중 분석 | Service 드롭다운에서 서비스 선택 → 검색 |
...
| 버전 | 날짜 | 변경 내용 |
|------|------|-----------|
| **1.4** | 2026-03-06 | Log Analyzer 패널 추가 (8절), 화면 구성 4-panel 레이아웃으로 갱신 (2절), 입력 영역 음성/파일 기능 반영 |
| **1.3** | 2026-02-20 | RAG 출처 인용 기능 추가 (7.5절), JsonElement 메타데이터 역직렬화 버그 수정 |
| **1.2** | 2026-02-20 | FileWatcher 자동 문서 수집 추가 (7.6절), 다중 모델 선택 (4절) |
| **1.1** | 2026-02-19 | RAG 지식 관리 (7절), Markdown/LaTeX 렌더링 (6절) |
| **1.0** | 2026-02-18 | 초기 버전 |
