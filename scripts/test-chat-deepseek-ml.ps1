# test-chat-deepseek-ml.ps1 - DeepSeek-R1 Multilingual 간단 테스트
...
try {
    Write-Host "=== DeepSeek-R1 Multilingual 7B Simple Test ===" -ForegroundColor Cyan
    $ws.ConnectAsync($uri, $cts.Token).GetAwaiter().GetResult()

    $convId = "test-dsml-" + [guid]::NewGuid().ToString("N").Substring(0,6)
    $msg = @{
        userMessage    = "2+2=? 숫자만 답해."
        conversationId = $convId
        modelId        = "kwangsuklee/DeepSeek-R1-Distill-Qwen-7B-Multilingual-Q5_K_M"
    ...
    Write-Host "Sent: '2+2=? 숫자만 답해.' (model: DeepSeek-R1-Multilingual)" -ForegroundColor Yellow
    Write-Host ""
