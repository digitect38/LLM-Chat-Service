# test-chat-deepseek-kr.ps1 - DeepSeek-R1 7B 한국어 RAG 테스트
...
try {
    Write-Host "=== DeepSeek-R1 7B Korean RAG Test ===" -ForegroundColor Cyan
    $ws.ConnectAsync($uri, $cts.Token).GetAwaiter().GetResult()

    $convId = "test-dskr-" + [guid]::NewGuid().ToString("N").Substring(0,6)
    $msg = @{
    ...
    Write-Host "(model: deepseek-r1:7b)" -ForegroundColor Yellow
    Write-Host ""
