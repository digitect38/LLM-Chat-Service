# test-chat-exaone4.ps1 - EXAONE 4.0 (1.2B) 성능 테스트
...
try {
    Write-Host "=== EXAONE 4.0 (1.2B) Speed Test ===" -ForegroundColor Cyan
    $ws.ConnectAsync($uri, $cts.Token).GetAwaiter().GetResult()

    $convId = "test-e4-" + [guid]::NewGuid().ToString("N").Substring(0,6)
    $msg = @{
    ...
    Write-Host "Sent: '2+2=? 숫자만 답해.' (model: ingu627/exaone4.0:1.2b)" -ForegroundColor Yellow
    Write-Host ""
