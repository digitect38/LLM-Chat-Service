...
public sealed class LlmEntityExtractor : IEntityExtractor
{
    ...
    private const string BatchExtractionPrompt = """
        당신은 반도체 FAB 장비 도메인 전문가입니다.
        주어진 텍스트에서 엔티티(개체)와 관계를 모두 추출하세요.
        ...
        반드시 JSON 형식으로만 응답하세요:
        {"entities": [{"name": "이름", "type": "타입"}], "relations": [{"source": "소스이름", "target": "타겟이름", "type": "관계타입"}]}

        설명이나 부가 텍스트 없이 JSON만 출력하세요.
        """;

    private const int BatchMaxChars = 1800;
    ...
    public async Task<(List<GraphEntity> Entities, List<GraphRelation> Relations)> ExtractFromBatchAsync(
        List<string> chunks, CancellationToken ct)
    {
        ...
        // Group chunks into batches of ~BatchMaxChars total
        var batches = BuildBatches(chunks, BatchMaxChars);

        _logger.LogDebug("Batch extraction: {ChunkCount} chunks grouped into {BatchCount} batches",
        ...
        foreach (var batch in batches)
        {
            try
            {
                var messages = new List<LlmChatMessage>
                {
                    LlmChatMessage.System(BatchExtractionPrompt),
                    LlmChatMessage.User(batch)
                };

                var response = await _llmClient.CompleteChatAsync(messages, null, ct);
                var (entities, relations) = ParseBatchResponse(response);
                allEntities.AddRange(entities);
                allRelations.AddRange(relations);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Batch entity extraction failed for a batch, continuing with next");
            }
        }

        _logger.LogDebug("Batch extraction completed: {EntityCount} entities, {RelationCount} relations",
            allEntities.Count, allRelations.Count);

        return (allEntities, allRelations);
    }

    internal static List<string> BuildBatches(List<string> chunks, int maxChars)
    {
        ...
        {
            var truncated = chunk.Length > maxChars ? chunk[..maxChars] : chunk;

            if (current.Length + truncated.Length + 2 > maxChars && current.Length > 0)
            {
                batches.Add(current);
                current = truncated;
            }
            ...
            {
                current = current.Length == 0 ? truncated : $"{current}\n\n{truncated}";
            }
        }
    ...
    internal List<GraphEntity> ParseEntities(string response)
    {
        try
        {
    ...
    internal List<GraphRelation> ParseRelations(string response, List<GraphEntity> entities)
    {
        try
        {
            var cleaned = StripCodeFences(response);
    ...
    private (List<GraphEntity> Entities, List<GraphRelation> Relations) ParseBatchResponse(string response)
    {
        try
        {
            var cleaned = StripCodeFences(response);
            ...
            _logger.LogDebug("Batch parsed {EntityCount} entities and {RelationCount} relations",
                entities.Count, relations.Count);
            return (entities, relations);
        }
        catch (JsonException ex)
        {
            _logger.LogWarning(ex, "Failed to parse batch extraction response");
            return ([], []);
        }
    }
}
